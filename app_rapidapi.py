...
def test_api(query_text):
    ...
    payload = {
        "messages": [
            {
                "role": "user",
                "content": f"Me dê uma ideia inovadora para uma startup sobre {query_text}. Responda em português, utilizando negrito para títulos e seções e '##' para subtítulos. Limite a resposta a no máximo 150 palavras. Foque somente na introdução da ideia."
            }
        ],
...
def generate_startup():
    ...
    if 'choices' in api_response and len(api_response['choices']) > 0:
        ...
        formatted_message = response_message.replace('#', '').strip()
